          ...
          <div className="flex-1 space-y-2">
            <div className="flex items-start justify-between">
              <div>
                ...
                <Link
                  ...
                  className="block font-semibold text-gray-900 hover:text-rose-600 transition-colors"
                >
                  {item.perfumes.name}
                </Link>
                <p className="text-lg font-bold text-rose-600">${item.perfumes.price}</p>
              </div>
              <Button
                variant="ghost"
                size="icon"
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                {item.quantity > 1 && <p className="text-xs text-gray-500">${item.perfumes.price} each</p>}
              </div>
            </div>

            {item.perfumes.stock_quantity < 10 && (
              <p className="text-xs text-amber-600">Only {item.perfumes.stock_quantity} left in stock</p>
            )}
          </div>
        </div>
      </CardContent>
